...
public final class DialogBuilder<T> {
    ...
    public DialogBuilder<T> withResourceBundleName(String resourceBundleName) {
        this.resourceBundle =resourceBundleManager.loadResourceBundle(resourceBundleName).getResourceBundle();
        return this;
    }
    ...
    public Dialog<T> build() {
        buttonEventHandlers.entrySet()
                .forEach(entry -> dialog.getDialogPane()
                                    .lookupButton(entry.getKey()).addEventHandler(ActionEvent.ACTION, entry.getValue()) );
        return dialog;
    }
