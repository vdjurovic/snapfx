...
public class DefaultProcessExecutor implements ProcessExecutor {
  ...
  @Override
  public Future<ProcessExecutionResult> executeExternalProcess(
  ...
  @Override
  public Future<Process> executeAndWait(
      List<String> cmdLine, File workingDirectory, Map<String, String> environment) {
    return executorService.submit(
        () -> executeProcessAndWait(cmdLine, workingDirectory, environment));
  }
  ...
  private void launchDefaultApplication(String target) throws ProcessExecutionException {
    ...
    var result =
        executeExternalProcess(cmdLine, new File(System.getProperty("user.dir")), Map.of());
    try {
  ...
  private ProcessExecutionResult executeProcess(
    ...
    LOGGER.info(
        "Running command {} in working directory {}", cmdLine, workingDirectory.getAbsolutePath());
  ...
  private Process executeProcessAndWait(
      List<String> cmdLine, File workingDirectory, Map<String, String> environment)
