...
public class DefaultPreferenceManager implements PreferenceManager {
    ...
    @Override
    public IntegerPreferenceEntry getIntegerPreference(String root, String name, Integer defaultValue) {
        var key = root + "-" + name;
    ...
    @Override
    public DoublePreferenceEntry getDoublePreferenceEntry(String root, String name, Double defaultValue) {
        var key = root + "-" + name;
        var cached = preferenceCache.get(key);
        if(cached != null) {
    ...
    public DoublePreferenceEntry getDoublePreferenceEntry(String name, Double defaultValue) {
        return getDoublePreferenceEntry(applicationConfig.defaultPreferenceRootNode(), name, defaultValue);
    }

    @Override
    public StringPreferenceEntry getStringPreferenceEntry(String root, String name, String defaultValue) {
        var key = root + "-" + name;
        var cached = preferenceCache.get(key);
        if(cached != null) {
    ...
    public StringPreferenceEntry getStringPreferenceEntry(String name, String defaultValue){
        return getStringPreferenceEntry(applicationConfig.defaultPreferenceRootNode(), name, defaultValue);
    }

    @Override
    public BooleanPreferenceEntry getBooleanPreferenceEntry(String root, String name, Boolean defaultValue) {
        var key = root + "-" + name;
        var cached = preferenceCache.get(key);
        if(cached != null) {
